    ...
    for (n = 1; n <= maxord; n++) {
      ...
      for (m = 0, D1 = 1, D2 = (n - m + D1) / D1; D2 > 0; D2--, m += D1) {
        ...
        if (m > 0) {
          flnmj = ((n - m + 1) * j) / (n + m);
          this.snorm[n + m * 13] =
            this.snorm[n + (m - 1) * 13] * Math.sqrt(flnmj);
          j = 1;
    ...
    for (let n = 1; n <= maxord; n++) {
      ...
      for (let m = 0, D3 = 1, D4 = (n + m + D3) / D3; D4 > 0; D4--, m += D3) {
        ...
        if (n === m) {
          ...
          this.dp[m][n] =
            ct * this.dp[m][n - 1] - st * this.snorm[n - 1 + m * 13];
        } else if (n > 1 && n !== m) {
          if (m > n - 2) this.snorm[n - 2 + m * 13] = 0.0;
          if (m > n - 2) this.dp[m][n - 2] = 0.0;
          this.snorm[n + m * 13] =
            ct * this.snorm[n - 1 + m * 13] -
            this.k[m][n] * this.snorm[n - 2 + m * 13];
          this.dp[m][n] =
            ct * this.dp[m][n - 1] -
        ...
        */
        const par = ar * this.snorm[n + m * 13];
        let temp1, temp2;
        if (m === 0) {
        ...
        if (st === 0.0 && m === 1) {
          if (n === 1) this.pp[n] = this.pp[n - 1];
            ...
            this.pp[n] =
              ct * this.pp[n - 1] - this.k[m][n] * this.pp[n - 2];
          const parp = ar * this.pp[n];
